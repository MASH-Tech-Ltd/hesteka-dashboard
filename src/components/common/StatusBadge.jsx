...
const StatusBadge = ({ status, config = {} }) => {
  ...
  const defaultStatusConfig = {
    ...
    expired: { label: t.expired || "Expired", color: "bg-red-100 text-red-600" },
    pending: {
  ...
  return (
    <span
      className={`text-[9px] font-bold px-2 py-0.5 rounded-full uppercase tracking-wider ${item.color}`}
    >
      {item.label}
    </span>
  );
