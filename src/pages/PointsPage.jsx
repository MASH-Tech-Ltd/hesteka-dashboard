    ...
    <div className="flex items-center justify-between p-4 border-b border-[#f0e8d8] last:border-0 hover:bg-[#fcfaf7] transition-all rounded-lg group">
      <div className="flex items-center gap-4">
        ...
        <div className="flex flex-col">
          <span className="text-[11px] font-bold text-[#3a2a1a] uppercase tracking-wider">
            {label}
          </span>
          <span className="text-[9px] text-[#9a8a7a] italic">
            {subtitleText ||
              t.currentValuePerAction ||
              "Current value per action"}
          </span>
        </div>
      </div>
      <div className="flex items-center gap-3">
        {isVariable ? (
          ...
          <div className="relative">
            ...
            />
            <div className="absolute -top-2 -right-2 bg-[#8B6914] text-white text-[8px] font-bold px-1.5 py-0.5 rounded-full shadow-sm">
              PTS
            </div>
          </div>
        )}
...
export default function PointsPage() {
  ...
  const fetchHistory = useCallback(async () => {
    ...
    try {
      const res = await api.get("/points/admin/history", {
      ...
      if (userSearchQuery.trim().length >= 2 && !selectedUser) {
        ...
        try {
          const res = await api.get(
            `/user/get-all-user?search=${userSearchQuery}&limit=5`,
          );
  ...
  const handleAssignCustomPoints = async () => {
    ...
    try {
      ...
      if (res.data.status === "ok" || res.data.success) {
        toast.success(
          t.pointsAssignedSuccess || "Points successfully assigned!",
        );
  ...
  const executeAssignAll = async () => {
    ...
    try {
      ...
      if (res.data.status === "ok" || res.data.success) {
        toast.success(
          `Points assigned to all ${res.data.data?.usersUpdated || "users"} successfully!`,
        );
        setCustomPoints("");
        setCustomPointsNote("");
        const statsRes = await api.get("/points/admin/stats");
      ...
      toast.error(
        err.response?.data?.message || "Failed to assign points to all users",
      );
      if (err.response?.data?.data && Array.isArray(err.response.data.data)) {
        err.response.data.data.forEach(e => toast.error(`Cause: ${e.message || JSON.stringify(e)}`));
      } else if (err.response?.data?.errors && Array.isArray(err.response.data.errors)) {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (configRes.data.status === "ok" || configRes.data.success) {
          ...
          if (
            fetchedConfig.isDoublePointsActive &&
            fetchedConfig.promotionEndTime
          ) {
  ...
  const handleUpdateConfig = async () => {
    ...
    try {
      ...
      if (res.data.status === "ok" || res.data.success) {
        toast.success(
          t.configUpdateSuccess || "Configuration updated successfully!",
        );
      }
    } catch (err) {
  ...
  const scale = [
    {
      key: "pointsPerReport",
      icon: <MapPin className="w-5 h-5 text-[#8B6914]" />,
      label: t.animalReports || "ANIMAL REPORTS",
      points: config?.pointsPerReport || 0,
    },
    {
      key: "pointsPerMission",
      icon: <Target className="w-5 h-5 text-[#8B6914]" />,
      label: t.localMissionsLabel || "LOCAL MISSIONS",
      isVariable: true,
      variableText: t.variable || "Variable",
      subtitleText: t.dependsOnMission || "Depends on specific mission",
    },
    {
      key: "pointsPerDonation",
  ...
  const rules = [
    {
      ...
      label:
        t.validityDuration && t.validityDuration.includes(config.validityMonths)
          ? t.validityDuration
    ...
    },
    {
      ...
      label:
        t.monthlyCeiling && t.monthlyCeiling.includes(config.monthlyCeiling)
          ? t.monthlyCeiling
          : `${t.monthlyCeiling || "Monthly ceiling"} - ${config.monthlyCeiling} pts`,
      active: config.isMonthlyCeilingActive,
    },
    {
      key: "isDoublePointsActive",
      label: t.doublePoints || "Double points active",
      active: config.isDoublePointsActive,
    },
    {
      key: "isPointsOnDonationsActive",
      label: t.pointsOnDonations || "Points on donations",
      active: config.isPointsOnDonationsActive,
    },
  ];
  ...
  const historyColumns = [
    ...
    },
    {
      ...
      cell: (tx) => (
        <span
          className={`font-black text-sm ${tx.points > 0 ? "text-green-600" : "text-red-600"}`}
        >
    ...
    },
    {
      ...
      cell: (tx) => (
        <span
          className={`px-2 py-1 rounded text-[10px] font-bold uppercase ${tx.type === "EARN" ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"}`}
        >
          {t[tx.type] || tx.type}
        </span>
      ),
    },
    {
      ...
      cell: (tx) => {
        ...
        const translationKey = `src${tx.source
          .split("_")
    ...
    },
    {
      ...
      cell: (tx) => (
        <span
          className="text-xs text-[#5a4a3a] max-w-[200px] truncate block"
          title={tx.note}
        >
          {tx.note || "—"}
        </span>
    ...
    },
    {
      ...
      cell: (tx) => (
        <div className="flex flex-col">
          ...
          <span className="text-[9px] text-[#9a8a7a] font-medium">
    ...
    <div className="px-6 py-4 flex flex-col gap-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard
          ...
          value={{
            text: stats?.distributed?.total?.toLocaleString() || "0",
            color: "text-green-600",
          }}
          sub={
            stats?.distributed?.thisMonth
              ? `+${stats.distributed.thisMonth.toLocaleString()} ${t.thisMonth || "This month"}`
        ...
        <StatCard
          ...
          value={{
            text: stats?.pending?.toLocaleString() || "0",
        ...
        <StatCard
          ...
          value={{
            text: stats?.exchanged?.total?.toLocaleString() || "0",
            color: "text-blue-600",
          }}
        ...
        <StatCard
          ...
          value={{
            text: stats?.expired?.toLocaleString() || "0",
            color: "text-red-600",
          }}
          sub={t.unused || "Unused"}
          subType="down"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white rounded-2xl border border-[#e8ddd0] flex flex-col overflow-hidden shadow-sm h-full">
          <div className="p-5 border-b border-[#e8ddd0] bg-[#fcfaf7] flex items-center justify-between">
            <h3 className="font-bold text-[#3a2a1a] text-sm flex items-center gap-2 uppercase tracking-tight">
              ...
              {t.pointsScale || "POINTS SCALE"}
            </h3>
            <div className="bg-[#8B6914]/10 text-[#8B6914] text-[9px] font-black px-3 py-1 rounded-full uppercase tracking-wider">
              {t.globalConfig || "GLOBAL CONFIGURATION"}
            </div>
          </div>
          ...
          <div className="p-6 mt-auto bg-[#fcfaf7] border-t border-[#e8ddd0] flex flex-row gap-4 items-stretch">
            <div className="bg-orange-50 border border-orange-100 p-4 rounded-xl flex gap-3 items-center flex-1">
              ...
              <p className="text-[10px] text-orange-800 font-bold leading-relaxed m-0">
                {t.configUpdateMsg ||
                  "Changes to the scale will be applied immediately for all future user actions. Existing points will not be affected."}
              </p>
            </div>
        ...
        <div className="bg-white rounded-2xl border border-[#e8ddd0] flex flex-col p-6 gap-6 shadow-sm h-full">
          <h3 className="font-bold text-[#3a2a1a] text-sm flex items-center gap-2 uppercase tracking-tight">
            <Settings className="w-4 h-4 text-[#8B6914]" />{" "}
            {t.usageRules || "USAGE RULES"}
          </h3>
          <div className="flex flex-col gap-6">
            {rules.map((rule, i) => (
              <div key={i} className="flex items-center justify-between group">
                <span className="text-xs font-bold text-[#3a2a1a] transition-colors">
                  {rule.label}
                </span>
                <button
                ...
                >
                  <div
                    className={`w-4 h-4 bg-white rounded-full shadow-md transform transition-transform duration-300 ${rule.active ? "translate-x-6" : "translate-x-0"}`}
                  />
                </button>
              </div>
            ))}
        ...
        <div className="bg-linear-to-br from-[#3a2a1a] to-[#2a1a0a] rounded-2xl p-6 text-white shadow-xl flex flex-col gap-3 relative overflow-hidden group h-full">
          ...
          <div className="flex items-center justify-between relative z-10">
            <div className="bg-[#8B6914] text-[8px] font-black px-2 py-0.5 rounded-full w-fit uppercase tracking-widest border border-white/20">
              {t.customPoints || "Custom Points"}
            </div>
              ...
              <div className="flex flex-col gap-1 relative">
                ...
                <div className="relative">
                  <input
                    ...
                    placeholder={
                      t.searchUserPlaceholder || "Type name or email..."
                    }
                    ...
                    {searchResults.map((user) => (
                      ...
                      >
                        ...
                        <div className="flex flex-col overflow-hidden">
                          <span className="text-xs font-bold text-[#3a2a1a] truncate">
                            {" "}
                            {user.firstName} {user.lastName}
                          </span>
                          <span className="text-[10px] text-[#9a8a7a] truncate">
                            {user.email}
                          </span>
                        </div>
                      </div>
            ...
            <div className="flex flex-col gap-2 mt-1">
              ...
              <div className="flex flex-col gap-1">
                ...
                <textarea
                  ...
                  className="bg-white/20 border border-white/30 rounded-lg px-2 py-2 text-xs text-white placeholder-white/50 focus:outline-none focus:ring-2 focus:ring-[#8B6914] focus:border-transparent resize-none"
                />
              </div>
            </div>
          </div>
      ...
      <div className="bg-white rounded-2xl border border-[#e8ddd0] flex flex-col overflow-hidden shadow-sm mt-4">
        <div className="p-5 border-b border-[#e8ddd0] bg-[#fcfaf7] flex items-center justify-between">
          <h3 className="font-bold text-[#3a2a1a] text-sm flex items-center gap-2 uppercase tracking-tight">
            <History className="w-4 h-4 text-[#8B6914]" />{" "}
        ...
        <FilterBar
          ...
          sortOptions={[
            ...
            {
              label: t.pointsDesc || "Points (High-Low)",
              value: "points:descending",
            },
            {
              label: t.pointsAsc || "Points (Low-High)",
              value: "points:ascending",
            },
          ]}
        />
      ...
      <ConfirmModal
        ...
        message={
          t.confirmAssignAllDesc ||
          `Are you absolutely sure you want to assign ${customPoints || 0} points to ALL registered users? This action cannot be easily undone.`
        }
